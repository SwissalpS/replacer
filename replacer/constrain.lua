...
replacer.max_time = tonumber(core.settings:get('replacer.max_time') or 1.0)
...
replacer.radius_factor = tonumber(core.settings:get('replacer.radius_factor') or 0.4)

-- disable minor modes on server
replacer.disable_minor_modes =
	core.settings:get_bool('replacer.disable_minor_modes') or false

-- priv to allow using history
replacer.history_priv = core.settings:get('replacer.history_priv') or 'creative'
...
replacer.history_disable_persistency =
	core.settings:get_bool('replacer.history_disable_persistency') or false
-- ignored when persistency is disabled. Interval in minutes to
replacer.history_save_interval =
	tonumber(core.settings:get('replacer.history_save_interval') or 7)
-- include mode when changing from history
replacer.history_include_mode =
	core.settings:get_bool('replacer.history_include_mode') or false
-- amount of items in history
replacer.history_max = tonumber(core.settings:get('replacer.history_max') or 7)
